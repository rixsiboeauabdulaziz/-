...
function MyOrders() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => setError(t("myOrders.error"))) // Перевод ошибки
      ...
      <div style={styles.page}>
        <div style={styles.container}>
          ...
          <div style={{ marginBottom: 36 }}>
            <p className="ord-overline">{t("myOrders.accountLabel")}</p>
            <h1 className="ord-title">{t("myOrders.title")}</h1>
          </div>

          {orders.length === 0 ? (
            <div className="ord-empty">
              <div className="ord-empty-icon">◻</div>
              <p className="ord-empty-text">{t("myOrders.empty")}</p>
              <button className="ord-btn-solid" onClick={() => navigate("/collections")}>
                {t("myOrders.toCollections")}
              </button>
            </div>
          ) : (
              ...
              {orders.map((order) => (
                <div key={order._id} className="ord-card">
                  ...
                  <div className="ord-card-header">
                    <div>
                      <p className="ord-id">{t("myOrders.orderId")} #{order._id.slice(-6).toUpperCase()}</p>
                      <p className="ord-date">
                        {new Date(order.createdAt).toLocaleDateString(t("myOrders.dateLocale"), {
                          day: "numeric", month: "long", year: "numeric"
                        })}
                      </p>
                    </div>
                    <span className="ord-status">{t("myOrders.statusAccepted")}</span>
                  </div>
                  ...
                  <div className="ord-product">
                    ...
                    <div className="ord-product-info">
                      <p className="ord-product-name">
                        {order.product?.name || t("myOrders.productDeleted")}
                      </p>
                      {order.product?.price && (
                        <p className="ord-product-price">
                          {Number(order.product.price).toLocaleString("ru-RU")} {t("myOrders.currency")}
                        </p>
                      )}
                      <p className="ord-product-qty">{t("myOrders.quantity")}: {order.quantity} {t("myOrders.pcs")}</p>
                    </div>
                    {order.product?.price && (
                      <div className="ord-total">
                        <p className="ord-total-label">{t("myOrders.totalLabel")}</p>
                        <p className="ord-total-value">
                          {(order.product.price * order.quantity).toLocaleString("ru-RU")} {t("myOrders.currency")}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="ord-contacts">
                    <div className="ord-contact-item">
                      <span className="ord-contact-label">{t("myOrders.recipient")}</span>
                      <span className="ord-contact-value">{order.name}</span>
                    </div>
                    <div className="ord-contact-item">
                      <span className="ord-contact-label">{t("myOrders.phone")}</span>
...
// ... стили (styles и css) остаются такими же ...
const styles = {
  page: {
    minHeight: "100vh",
    background: "#0a0a0a",
...
const css = `
  ...
  .ord-card { background:#111; border:1px solid rgba(255,255,255,.07); border-radius:4px; padding:28px; transition: border-color .2s; }
  ...
  .ord-status { font-size:.55rem; font-weight:600; letter-spacing:.2em; text-transform:uppercase; background:rgba(29,158,117,.12); border:1px solid rgba(29,158,117,.3); color:#5dcaa5; padding:4px 12px; border-radius:2px; white-space:nowrap; }
  ...
  .ord-product-img { width:72px; height:72px; object-fit:cover; border-radius:2px; border:1px solid rgba(255,255,255,.08); flex-shrink:0; }
  ...
  .ord-empty { display:flex; flex-direction:column; align-items:center; justify-content:center; padding:80px 24px; text-align:center; border:1px solid rgba(255,255,255,.06); border-radius:4px; background:#111; }
  ...
  .ord-btn-solid { background:#D4AF37; border:1px solid #D4AF37; color:#0a0a0a; font-family:'Montserrat',sans-serif; font-size:.65rem; font-weight:600; letter-spacing:.18em; text-transform:uppercase; padding:12px 28px; border-radius:2px; cursor:pointer; transition:opacity .2s; }
  .ord-btn-solid:hover { opacity:.85 }
  @media(max-width:600px) { .ord-product { flex-wrap:wrap } .ord-total { text-align:left } .ord-card-header { flex-direction:column; gap:12px } }
`
