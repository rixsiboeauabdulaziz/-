...
function Register() {
  ...
  const fileObjRef = useRef(null) // ✅ Храним настоящий файл отдельно
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (err) => {
      // ✅ Правильно читаем ошибку от сервера
  ...
  const handleFile = (e) => {
    ...
    fileObjRef.current = file // ✅ Сохраняем настоящий File-объект

    const reader = new FileReader()
    reader.onload = () => {
      setPreview(reader.result) // ✅ Теперь превью работает
    }
  ...
  const handleSubmit = (e) => {
    ...
    // ✅ Собираем FormData здесь, с настоящим файлом
    ...
    if (fileObjRef.current) {
      data.append("img", fileObjRef.current) // ✅ Multer получит настоящий файл
    }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <p style={s.eyebrow}>Создать аккаунт</p>
        <h1 style={s.heading}>Регистрация</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div style={s.avatarWrap}>
            ...
            <span style={s.avatarHint}>Фото профиля</span>
            <input
              ref={fileRef}
              type="file"
              accept="image/*"
          ...
          <button
            type="submit"
            disabled={isPending}
            style={{ ...s.btn, opacity: isPending ? 0.7 : 1, cursor: isPending ? "not-allowed" : "pointer" }}
            onMouseEnter={(e) => { if (!isPending) e.currentTarget.style.opacity = "0.8" }}
