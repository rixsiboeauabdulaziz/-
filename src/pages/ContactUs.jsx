...
const ContactUs = () => {
  ...
  const channels = [
    {
      label: t('contacts.page.phone'),
      value: PHONE,
      hint: t('contacts.page.phone_hint'),
      href: PHONE_HREF,
      svg: (
    ...
    },
    {
      ...
      hint: t('contacts.page.email_hint'),
  ...
  const handleSend = () => {
    ...
    const text = `${t('contacts.name')}: ${form.name}%0A${t('contacts.phone')}: ${form.contact}%0A${t('contacts.page.message_label')}: ${form.message}`;
    ...
    <div style={{ background: CREAM, fontFamily: "'DM Sans', sans-serif", overflowX: 'hidden' }}>
      ...
      <section style={{ position: 'relative', height: '60vh', minHeight: '440px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', padding: '0 2.5rem 5rem' }}>
          ...
          <h1 style={{
            ...
            {t('contacts.page.hero_title_1')}<br /><em style={{ color: GOLD }}>{t('contacts.page.hero_title_2')}</em>
          </h1>
          <p style={{
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '7rem 2.5rem' }}>
        <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7rem', alignItems: 'start' }}>
          ...
          {/* FORM */}
          <Reveal>
            <p style={{ color: GOLD, letterSpacing: '0.45em', fontSize: '0.6rem', textTransform: 'uppercase', marginBottom: '0.6rem', fontWeight: 500 }}>{t('contacts.label')}</p>
            <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 'clamp(1.8rem, 3vw, 2.5rem)', fontWeight: 400, color: DARK, lineHeight: 1.2, marginBottom: '2.8rem' }}>
              {t('contacts.page.form_title_1')}<br /><em style={{ color: GOLD }}>{t('contacts.page.form_title_2')}</em>
            </h2>
            ...
            {sent ? (
              <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                ...
                <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '1.3rem', color: DARK, marginBottom: '0.5rem' }}>{t('contacts.page.sent_title')}</p>
                <p style={{ color: WARM_GRAY, fontSize: '0.83rem', fontWeight: 300 }}>{t('contacts.page.sent_desc')}</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                <input type="text" placeholder={t('contacts.name')} value={form.name}
                  onChange={e => setForm({ ...form, name: e.target.value })}
                  onFocus={() => setFocused('name')} onBlur={() => setFocused('')}
                  style={inputStyle('name')} />
                <input type="text" placeholder={t('contacts.phone')} value={form.contact}
                  onChange={e => setForm({ ...form, contact: e.target.value })}
                  onFocus={() => setFocused('contact')} onBlur={() => setFocused('')}
                  style={inputStyle('contact')} />
                <textarea rows={4} placeholder={t('contacts.page.message_placeholder')} value={form.message}
          ...
          <Reveal delay={0.18}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
              <div>
                <p style={{ color: GOLD, fontSize: '0.62rem', letterSpacing: '0.3em', textTransform: 'uppercase', fontWeight: 500, marginBottom: '1rem' }}>{t('contacts.page.showroom')}</p>
                <p style={{ fontSize: '1.05rem', color: DARK, fontWeight: 400, marginBottom: '0.4rem' }}>{t('contacts.addressValue')}</p>
                <p style={{ color: WARM_GRAY, fontSize: '0.83rem', fontWeight: 300, lineHeight: 1.7 }}>{t('contacts.hoursValue')}<br />{t('contacts.page.sat_hours')}</p>
              </div>

              <div style={{ height: '1px', background: 'rgba(0,0,0,0.08)' }} />

              <div>
                <p style={{ color: GOLD, fontSize: '0.62rem', letterSpacing: '0.3em', textTransform: 'uppercase', fontWeight: 500, marginBottom: '1.2rem' }}>{t('contacts.page.socials')}</p>
              ...
              <div style={{ background: DARK, borderRadius: '2px', padding: '2rem' }}>
                <p style={{ color: GOLD, fontSize: '0.6rem', letterSpacing: '0.3em', textTransform: 'uppercase', fontWeight: 500, marginBottom: '0.8rem' }}>{t('delivery.cta_label')}</p>
                {[
                  [t('contacts.page.phone') + ' / WhatsApp', t('contacts.page.time_now')],
                  ['Telegram', t('contacts.page.time_15')],
                  ['Email / ' + t('contacts.page.form_short'), t('contacts.page.time_1h')],
                ].map(([ch, time]) => (
      ...
      <div style={{ background: DARK, padding: '2.5rem 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.7rem', letterSpacing: '0.1em' }}>{t('delivery.footer_copy')}</p>
        <div style={{ display: 'flex', gap: '2rem' }}>
          {['home', 'about', 'delivery'].map(key => (
